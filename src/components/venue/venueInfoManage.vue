<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 用户列表 -->
      <el-table :data="venueInfoList" :default-sort="{ prop: 'id', order: 'ascending' }"
        style="width: 75%;text-align:center;font-size:15px">
        ...
        <el-table-column align="center" label="负责人" prop="">
          <template slot-scope="scope">
            <el-button type="text" @click="showCharge(scope.$index)" size="medium">
              <!-- {{ scope.row.venueCharge.name }} -->
            </el-button>
          </template>
        </el-table-column>
      ...
      <!-- 分页 -->
      <el-pagination style="margin-left:400px;margin-top:20px" @size-change="handleSizeChange"
        @current-change="handleCurrentChange" :current-page="queryInfo.pageNum" :page-sizes="[10, 20]"
    ...
    <!-- 新建场地对话框 -->
    <el-dialog title="新建场地信息" :visible.sync="addDialogVisible" width="30%" @close="addDialogVisible = false"
      class="addVenueShow">
      <!-- 内容主体区域 -->
      <el-form :model="newVenueInfo" :rules="addFormRules" ref="addFormRef" label-width="125px" class="addVenueForm">
        <!-- 选择场地类别 -->
        <el-form-item label="场地信息" prop="venueId" style="margin-top:20px">
          <el-select v-model="newVenueInfo.venueId" placeholder="请选择场地" style="width:85%">
            <!-- <el-option v-for="item in venueList" :key="item.id"
              :label="item.id + '-' + item.venueClass + '-' + item.position + '-' + item.venueCharge.name + '-' + item.status"
              :value="item.id">
            </el-option> -->
            <el-option v-for="item in venueList" :key="item.id"
              :label="item.id + '-' + item.venueClass + '-' + item.position + '-' + '-' + item.status" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
        <!-- 选择场地位置 -->
        <el-form-item label="场地可预约时间" prop="periodId">
          <el-select v-model="newVenueInfo.periodId" placeholder="请选择时间段" style="width:85%">
            <el-option v-for="item in venuePeriodList" :key="item.id" :label="item.period" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
